/**
 * Batch operations for podcasts.
 *
 * @route PATCH /api/podcasts/batch — Batch update sort orders (admin/superadmin)
 */
import type { NextRequest } from 'next/server';
import { NextResponse } from 'next/server';
import { prisma } from '@/lib/db';
import { ApiError, createErrorResponse, badRequest, internalError } from '@/lib/api/errors';
import { batchUpdateSortOrderSchema } from '@/lib/schemas/podcast';
import { requireAuth, requireRole } from '@/lib/auth/api-helpers';

/**
 * Batch updates podcast sort orders within a transaction.
 *
