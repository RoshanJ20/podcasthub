/**
 * PodcastMainContent — the primary left-column content area for the podcast detail page.
 *
...
/** Props consumed by PodcastMainContent. */
export interface PodcastMainContentProps {
  /** Whether to render the compact (PDF-open) layout instead of the full layout. */
  compact: boolean;
  /** ID of the podcast, used to scope bookmarks. */
  podcastId: string;
  /** Resolved thumbnail URL for the hero image in full mode. */
  thumbnailUrl: string;
  /** Podcast title rendered as the page heading in full mode. */
  title: string;
  /** Podcast description shown beneath the title in full mode. */
  description: string;
...
/**
 * Renders either the full or compact left-column content for the podcast detail page.
 *
 ...
 * @param props.podcastId - Podcast ID used to fetch and save bookmarks.
 * @param props.thumbnailUrl - Thumbnail asset URL for the hero image.
 * @param props.title - Podcast title for the h1 heading.
 * @param props.description - Podcast description paragraph.
  ...
  if (compact) {
    return (
      <motion.div key="compact" {...mercuryIn}>
        ...
        <div className="mt-3">
          <BookmarkPanel podcastId={podcastId} onSeek={onSeek} domainColor={domainColor} compact />
        </div>
      </motion.div>
    );
  }
