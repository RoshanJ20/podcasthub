/**
 * Unit tests for PodcastUploadWizard orchestrator component.
 *
 * Verifies:
 ...
 *
 * @dependencies vitest, @testing-library/react, PodcastUploadWizard
 */
...
async function fillAndAdvanceTo(
  ...
  await user.type(screen.getByPlaceholderText('Podcast title'), 'Test Podcast');
  await user.type(screen.getByPlaceholderText('Podcast description'), 'A test description');
...
describe('PodcastUploadWizard', () => {
  it('renders step indicator at step 1 (Details) initially', () => {
    render(<PodcastUploadWizard />);
