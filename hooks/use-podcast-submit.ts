/**
 * Custom hook encapsulating the submit logic for the podcast upload/edit form.
 *
 ...
 * - Submits the assembled podcast payload to the appropriate API endpoint
 *   (POST /api/podcasts for create, PUT /api/podcasts/:id for edit).
 * - Saves short and long transcript text to /api/podcasts/:id/transcript when
...
import type { PodcastFormData } from '@/components/admin/podcast-upload-form';
...
const log = createLogger('use-podcast-submit');
...
interface UsePodcastSubmitOptions {
  /** Whether to POST (create) or PUT (edit) the podcast. */
  mode: 'create' | 'edit';
  /** Pre-existing podcast data used in edit mode. */
  ...
  /** Called after a successful podcast save (create or edit). */
  onSuccess?: () => void;
}

interface UsePodcastSubmitReturn {
  /**
   * RHF-compatible submit handler.
   * Pass directly to `handleSubmit(onSubmit)` in the form.
   */
  onSubmit: (data: PodcastFormValues) => Promise<void>;
  /** True while upload or API requests are in flight. */
  isSubmitting: boolean;
}

/**
 * Returns a submit handler and submitting state for the podcast upload/edit form.
 *
 ...
 */
export function usePodcastSubmit({
  mode,
  initialData,
  tags,
  thumbnailFile,
  ...
  bulletinUpload,
  onSuccess,
}: UsePodcastSubmitOptions): UsePodcastSubmitReturn {
  const [isSubmitting, setIsSubmitting] = useState(false);

  const onSubmit = async (data: PodcastFormValues): Promise<void> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        log.warn({ error: body.message ?? String(body) }, 'Podcast save failed');
